...
- Per-user OAuth connectors (Strava, Fitbit, Exist): `STRAVA_CLIENT_ID` / `STRAVA_CLIENT_SECRET`, `FITBIT_CLIENT_ID` / `FITBIT_CLIENT_SECRET`, `EXIST_CLIENT_ID` / `EXIST_CLIENT_SECRET`. Each provider is gated by `providerConfigured()` (both id + secret required): the connect/sync routes return 503 and the status endpoint reports `configured:false` until its pair is set, so the feature ships safely before credentials exist. Optional `OAUTH_TOKEN_KEY` (AES-256-GCM token sealing) and `OAUTH_STATE_SECRET` (HMAC CSRF state) both fall back to a `REPL_ID`-derived key; set them explicitly in production.
