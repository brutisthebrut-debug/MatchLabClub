...
- Echo SMS (Twilio): `TWILIO_ACCOUNT_SID` / `TWILIO_AUTH_TOKEN` / `TWILIO_FROM_NUMBER` for outbound SMS, plus `TWILIO_VERIFY_SERVICE_SID` for the phone-verification OTP flow — logs instead of sending when unset; only texts users who opted in. The `twilio` package is a runtime dependency loaded via a variable-specifier dynamic import, so an absent package or missing creds degrades to the log transport without crashing.
- Per-user OAuth connectors (a growing library of REAL OAuth2 providers): `STRAVA_CLIENT_ID` / `STRAVA_CLIENT_SECRET`, `FITBIT_CLIENT_ID` / `FITBIT_CLIENT_SECRET`, `EXIST_CLIENT_ID` / `EXIST_CLIENT_SECRET`, `SPOTIFY_CLIENT_ID` / `SPOTIFY_CLIENT_SECRET`, `OURA_CLIENT_ID` / `OURA_CLIENT_SECRET`, `TRAKT_CLIENT_ID` / `TRAKT_CLIENT_SECRET`, `GITHUB_CLIENT_ID` / `GITHUB_CLIENT_SECRET`, `REDDIT_CLIENT_ID` / `REDDIT_CLIENT_SECRET`, `DISCORD_CLIENT_ID` / `DISCORD_CLIENT_SECRET`. Each provider is a single entry in `OAUTH_PROVIDERS` (`lib/oauthConnectors.ts`) with a real `fetchAndReduce` (real API call, no mocks) that reduces the response to a single derived count. Every provider is gated by `providerConfigured()` (both id + secret required): the connect/sync routes return 503 and the status endpoint reports `configured:false` until its pair is set, so each provider ships safely before its credentials exist. Optional `OAUTH_TOKEN_KEY` (AES-256-GCM token sealing) and `OAUTH_STATE_SECRET` (HMAC CSRF state) both fall back to a `REPL_ID`-derived key; set them explicitly in production.
...
- **Connection Center is the spine of the deep product** — `/connections` (`pages/ConnectionCenter.tsx`) is the single hub for every data source. Each connector is a card with the same shape: name, status (live / building / researching), what it returns, what we'll see, what we'll never touch, and a CTA to the real page. Every new data signal (Plaid, forwarding inbox, calendar, Spotify, etc.) lives here and follows this contract. `/integrations` (`pages/Integrations.tsx`) is a separate marketing "Platform Map" (OAuth roadmap for Hinge/Tinder/Bumble + waitlist) — keep the two distinct. Connector roadmap in Beats: 1 = Connection Center repositioning (shipped); 2 = per-user forwarding inbox at `{handle}@receipts.matchlab.club` (subject + sender + timestamp only, never the body, sidesteps Google CASA); 3 = Plaid spending; 4 = insight stream on `/me`; 5 = calendar `.ics` paste (shipped, wired into Match Readiness as the `calendar` lane, reads only the derived event count); 6 = signal-density visualisation; 7 = matching cohort opt-in. Each beat ships standing alone. Real per-user OAuth connectors are a library of live cards here: connect/callback/sync/disconnect routes in `routes/connectors.ts`, provider config + fetch-and-reduce in `lib/oauthConnectors.ts`, AES-256-GCM token sealing (`tokenCrypto.ts`) into the `oauth_tokens` table, HMAC CSRF state (`oauthState.ts`). Adding a provider = one `OAUTH_PROVIDERS` entry (real `fetchAndReduce` + `providerConfigured()` gate) + one line in `OAUTH_PROVIDER_IDS` and one card in `ConnectionCenter.tsx` (which auto-renders the generic `OAuthConnectorPanel`). Lane mapping: Strava + Fitbit + Oura feed the `vitality` lane (via its `sources[]`); Spotify feeds `music`; Trakt feeds `film`; GitHub + Reddit + Discord feed the `communities` lane; Exist is its own lane. When a lane lists multiple `sources[]`, `signalCounts.ts` sums the latest per-source count generically (no per-lane code). Only a per-source `{counts:{items}}` summary is ever persisted — raw provider payloads are reduced in-process and discarded. Backbone supports per-provider token quirks: `tokenAuth` basic/body, `tokenContentType` json (Trakt), `tokenHeaders` (GitHub/Reddit User-Agent), `refreshNeedsRedirectUri` (Trakt), and `extraAuthParams` (Reddit `duration=permanent`).
