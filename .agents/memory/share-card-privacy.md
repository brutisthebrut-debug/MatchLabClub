...
**Mirror snippet extension (shareable moments):** the card may also show one OPTIONAL, user-chosen line from the Mirror portrait (`portrait.headline` or a `portrait.known[].insight`). These are safe because `buildMirrorPortrait` derives them from registry coverage + labels, never from raw bios/messages/names. Any future snippet source MUST stay on this derived-only path; never source a share snippet from stored content. Selection is explicit (None / Overview / per-lane), defaults to "headline", and the live card is the user's confirmation of exactly what ships.
