...
const FELT_OFF_CHIPS = [
  ...
  "Felt judged",
  "Too much pressure",
  "Something was off but I can't say what",
];
const OUTCOMES = [
  ...
  "Just processing, no outcome yet",
];
...
function buildFallback(
  ...
  const goodChem =
    feltGood.includes("Good chemistry") || feltGood.includes("Real connection");
  const selfConsc =
    feltOff.includes("Felt judged") || feltOff.includes("Too much pressure");

  let patternRead = "";
  if (wasPositive && goodChem) {
    patternRead =
      "This one had real signal, the combination of good chemistry and feeling like yourself is worth paying attention to. That doesn't happen by default.";
  } else if (mixedSignals || (!wasPositive && feltGood.length > 0)) {
    patternRead =
      "The mixed signals you noticed are worth sitting with. When something feels good in some ways and off in others, it usually means one of two things: the timing isn't right, or the fit is real but the circumstances need to change.";
  } else if (oneSided) {
    patternRead =
      "One-sided conversations are a pattern worth tracking. It can come from nerves on their end, or from a mismatch in how much each person is investing. Either way, it's data, not a verdict.";
  } else if (selfConsc) {
    patternRead =
      "The feeling of being judged or under pressure usually says more about the environment than about you. Some situations just have that energy, and the right person won't create it.";
  } else if (wasPositive) {
    patternRead =
      "On balance, this sounds like it went well. The things you noticed going right are the real signal, they're what to look for more of.";
  } else {
    patternRead =
      "Not every interaction gives you clear signal, and that's fine. Sometimes the most useful debrief is just: what do I want more of? What was missing?";
  }

  const outcomeText = outcome || "still processing";
  const coachInsight = `Outcome: ${outcomeText}. ${
    outcome.includes("continuing")
      ? "Keep the energy consistent, don't over-invest before you've had more time together."
      : outcome.includes("fizzled")
        ? "Fading out naturally is normal data. If you wanted it to continue, the question is: what would one direct, low-stakes message have looked like?"
        : outcome.includes("ended it")
          ? "Knowing what you don't want is as useful as knowing what you do. Note what made you decide."
          : outcome.includes("they ended")
            ? "When someone else ends it, the instinct is to find what went wrong. Sometimes the answer is: it just wasn't the right match."
            : "Processing without an outcome is completely valid. Give it 24 hours before you decide what (if anything) to do next."
  }`;

  const nextStep =
    wasPositive && outcome.includes("continuing")
      ? {
          label: "Matches",
          href: "/matches",
          action: "Return to the connection when you are ready.",
        }
      : {
          label: "Journey",
          href: "/journey",
          action: "Keep this reflection with the rest of your real history.",
        };
...
function tryParseAi(raw: string | undefined, fb: DebriefResult): DebriefResult {
  ...
  try {
    ...
    return {
      patternRead:
        typeof p.patternRead === "string" && p.patternRead.length > 10
          ? p.patternRead
          : fb.patternRead,
      coachInsight:
        typeof p.coachInsight === "string" && p.coachInsight.length > 10
          ? p.coachInsight
          : fb.coachInsight,
...
export default function DebriefWhatHappened() {
  useMeta(
    ...
    "Reflect on an interaction, identify what felt good or confusing, and choose a clear next step.",
  );
  ...
  function mapOutcome(o: string): PostDateOutcome | undefined {
    ...
    if (o.includes("ended it") || o.includes("fizzled"))
      return PostDateOutcome.no_more;
  ...
  function handleGenerate() {
    ...
    enhance.mutate(
      { data: { toolName: "Debrief", prompt, expectJson: true } },
      {
        onSuccess: (data) => {
          const raw = (data as { output?: string } | undefined)?.output;
          setResult(tryParseAi(raw, fb));
        },
      },
    );
  }
  ...
  const canGenerate =
    what.trim().length > 0 || feltGood.length > 0 || feltOff.length > 0;
        ...
        <div className="max-w-2xl mx-auto relative z-10">
          ...
          <motion.div {...fadeUp(0.03)} className="mb-6">
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-sm font-medium text-[hsl(348_55%_75%)]">
                Echo
              </p>
            </div>
            <h1 className="text-3xl font-bold text-foreground">
              Tell me how it really felt
            </h1>
            <p className="text-muted-foreground mt-1.5 text-sm leading-relaxed">
              I will help you separate chemistry, comfort, uncertainty, and
              wishful thinking. Nothing becomes profile truth until you approve
              it.
            </p>
          </motion.div>
          ...
          >
            {[0, 1].map((s) => (
              <div
                key={s}
                className={`h-1.5 rounded-full transition-all ${s === step ? "flex-1 bg-[hsl(348_55%_65%)]" : "w-6 bg-white/10"}`}
              />
            ))}
          ...
          <AnimatePresence mode="wait">
            {step === 0 ? (
              <motion.div
                key="form"
                {...fadeUp(0.06)}
                className="glass border border-white/8 rounded-3xl p-6 sm:p-7 space-y-5"
              >
                <div className="space-y-2">
                  <Label className="text-foreground/70 text-xs font-semibold uppercase tracking-wider">
                    ...
                    <span className="font-normal normal-case text-muted-foreground/40">
                      (brief, a date, a conversation, a message exchange)
                    </span>
                  </Label>
                  <Textarea
                    value={what}
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {FELT_GOOD_CHIPS.map((c) => (
                      <button
                        key={c}
                        onClick={() => toggleChip(feltGood, setFeltGood, c)}
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {FELT_OFF_CHIPS.map((c) => (
                      <button
                        key={c}
                        onClick={() => toggleChip(feltOff, setFeltOff, c)}
              ...
              <motion.div key="results" {...fadeUp(0.05)} className="space-y-4">
                {enhance.isPending && (
                  <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[hsl(348_55%_65%/0.08)] border border-[hsl(348_55%_65%/0.2)] text-xs text-[hsl(348_55%_75%)]">
                    <Loader2 className="w-3.5 h-3.5 animate-spin" /> Echo is
                    reading the pattern…
                  </div>
                )}

                {createNote.isPending && (
                  <div className="rounded-xl border border-white/10 bg-white/[0.03] px-4 py-2.5 text-xs text-muted-foreground">
                    Saving this privately to your Journey...
                  </div>
                )}
                {saveError && (
                  <div className="rounded-xl border border-[hsl(350_70%_60%/0.3)] bg-[hsl(350_70%_60%/0.08)] px-4 py-2.5 text-xs text-foreground">
                    <p>
                      This reflection is still on screen, but it was not saved.
                      Try again before leaving.
                    </p>
                    <button
                      type="button"
                      className="mt-2 font-semibold text-[hsl(350_80%_78%)] underline underline-offset-2"
                      onClick={() => {
                        setSaveError(false);
                        setStep(0);
                      }}
                    >
                      Return to the debrief
                    </button>
                  </div>
                )}
                {savedNoteId !== null && (
                  <div className="rounded-xl border border-[hsl(142_55%_60%/0.25)] bg-[hsl(142_55%_60%/0.08)] px-4 py-2.5 text-xs text-foreground">
                    Saved privately. I have a tentative learning ready for your
                    review.
                  </div>
                )}
                ...
                <div className="rounded-2xl border border-[hsl(348_55%_65%/0.2)] bg-[hsl(348_55%_65%/0.06)] p-5">
                  <p className="text-xs font-semibold uppercase tracking-wider text-[hsl(348_55%_65%)] mb-2">
                    Suggested next step
                  </p>
                  <p className="text-sm text-muted-foreground mb-3">
                    {savedNoteId !== null
                      ? "Review the learning I took from this. You can confirm it, rewrite it, or dismiss it."
                      : show.nextStep.action}
                  </p>
                  <Link
                    href={
                      savedNoteId !== null
                ...
                <ToolHandoff
                  ...
                  fedLine="Your debrief stays private. Echo proposes a learning from it, and you decide whether that learning belongs in your profile."
                  steps={[
                    {
                      label: "Return to Matches",
                      href: connectionId
                        ? `/matches/${connectionId}`
                        : "/matches",
                      desc: "Continue the real connection at your pace.",
                    },
                    {
                      label: "See your Journey",
                      href: "/journey",
                      desc: "Keep this reflection with your durable history.",
                    },
                    {
                      label: "Talk with Echo",
                      href: "/echo",
                      desc: "Confirm what Echo noticed before it changes your profile.",
                    },
                  ]}
                />
