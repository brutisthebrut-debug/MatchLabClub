...
import { createBillingCheckout } from "@workspace/api-client-react";
import { useAuth } from "@workspace/replit-auth-web";
import {
  CheckCircle2, Sparkles, Lock, ArrowRight, Loader2,
...
function PaidForm({ product }: { product: Product }) {
  ...
  data-testid={`button-checkout-${product}`}
  >
  {redirecting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Lock className="w-4 h-4" />}
  {isAuthenticated ? config.cta : "Sign in to continue"}
  </button>
