...
describe("member data registry", () => {
  it("has one explicit privacy disposition per registered table", () => {
    expect(new Set(MEMBER_DATA_TABLE_NAMES).size).toBe(
      MEMBER_DATA_TABLE_NAMES.length,
    );
    for (const entry of MEMBER_DATA_REGISTRY) {
      expect(entry.export).toMatch(/^(full|sanitized|omit-security-secret)$/);
  ...
  it("rejects schema drift for every directly user-linked table", () => {
    ...
    for (const value of Object.values(schema)) {
      try {
        ...
        // The schema barrel also exports enums and helpers; only tables have
        // names and columns understood by Drizzle.
      }
    }

    const registered = new Set(MEMBER_DATA_TABLE_NAMES);
    expect([...discovered].filter((name) => !registered.has(name))).toEqual([]);
    expect(registered.has("audit_report_versions")).toBe(true);
    expect(registered.has("purchase_interest")).toBe(true);
  });
});
