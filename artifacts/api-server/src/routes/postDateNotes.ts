...
router.get("/post-date-notes", async (req, res): Promise<void> => {
  ...
  if (view === "active")
    filters.push(isNull(postDateNotesTable.deletedAt) as SQL);
  else if (view === "trash")
    filters.push(isNotNull(postDateNotesTable.deletedAt) as SQL);
  if (q.length > 0) {
...
router.post("/post-date-notes", async (req, res): Promise<void> => {
  ...
  if (connectionId) {
    ...
    if (!connection.dateCompletedAt) {
      res
        .status(409)
        .json({ error: "Mark the date complete before saving a debrief" });
      return;
    }
    ...
    if (existingDebrief) {
      res
        .status(409)
        .json({ error: "You already saved a debrief for this date" });
      return;
    }
...
router.patch("/post-date-notes/:id", async (req, res): Promise<void> => {
  ...
  if (parsed.data.whatDidnt !== undefined)
    patch.whatDidnt = parsed.data.whatDidnt;
  ...
  if (parsed.data.outcome !== undefined)
    patch.outcome = parsed.data.outcome ?? null;
