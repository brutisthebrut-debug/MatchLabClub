...
function renderExpiredExportPage(req: Request): string {
  const accountUrl = `${getOrigin(req)}/account`;
  return `<!doctype html>
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1" />
    <title>Export link expired — Next Level Dating Club</title>
  ...
  <body>
    <main class="card" role="main">
      ...
      <h1>This export link can&rsquo;t be used anymore</h1>
      <p>
        Data export links are single-use and expire after 30 minutes for your
        security. This one has either already been opened, expired, or
        we don&rsquo;t recognize it.
      </p>
      <p>
        No worries &mdash; you can request a fresh export link from your
        account page anytime.
      </p>
      <a class="cta" href="${accountUrl}">Request a new export link</a>
...
function sendExpiredExport(req: Request, res: import("express").Response): void {
  if (wantsHtml(req)) {
    ...
    res.send(renderExpiredExportPage(req));
    return;
  }
