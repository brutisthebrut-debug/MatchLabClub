...
router.get(
  ...
  async (_req, res): Promise<void> => {
    ...
    const [usersWithAnswers] = await db
      .select({
        c: sql<number>`count(distinct ${wellnessAnswersTable.userId})::int`,
      })
      .from(wellnessAnswersTable);
    ...
    res.json({
      ...
      topDimensions: dimensionCounts
        .slice(0, 6)
        .map((d) => ({ dimension: d.dimension, count: Number(d.c) })),
    });
  },
);
...
export function buildAlertReason(args: {
  ...
  const {
    ...
    recentRetried,
    recentFirstTrySuccessRate,
  } = args;
  const fallbackPct =
    recentTotal > 0 ? Math.round((recentFallbacks / recentTotal) * 100) : 0;
  ...
  if (
    ...
    recentValidationFailures >= recentRetried
  ) {
...
router.get(
  ...
  `);
    ...
    const norm = (row: (typeof perTool)[number]) => {
      ...
      const recentFirstTryOk = sliced.filter(
        (r) => r.attempts === 1 && !r.isFallback,
      ).length;
      const recentFallbacks = sliced.filter((r) => r.isFallback).length;
      const recentValidationFailures = sliced.filter(
        (r) => !r.validated,
      ).length;
      const recentRetried = sliced.filter(
        (r) => !r.isFallback && r.attempts > 1,
      ).length;
    ...
    const cooldownStates = alertStates
      ...
      .map((s) => {
        ...
        const remainingMs = Math.max(
          0,
          cooldownEndsAt.getTime() - now.getTime(),
        );
    ...
    res.json({
      overall: {
        ...
        overallSuccessRate:
          totalRows > 0 ? (totalFirstTry + totalRetried) / totalRows : 0,
      ...
      perToolOverrides: Array.from(perToolCfg.entries()).map(
        ([toolName, c]) => ({
          toolName,
      ...
      alerts: alerts.map((t) => {
        ...
        const suppressedByCooldown =
          cooldownToolNames.has(t.toolName) && (state?.breached ?? false);
        return {
          toolName: t.toolName,
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    const clearing = tierRaw === null || tierRaw === "";
    const tier = clearing ? null : normalizePlanGrant(tierRaw);
    if (!clearing && tier === null) {
      res.status(400).json({
        error:
          "Plan must be one of: member, insight, match, guided, or null to clear.",
      });
      return;
    }
    const now = new Date();
    ...
    if (tier && PAID_PLAN_KEYS.includes(tier)) {
    ...
    res.json({
      user: {
        ...
        email: row.email,
        tier: row.tier,
        tierGrantedAt: row.tierGrantedAt
          ? row.tierGrantedAt.toISOString()
          : null,
      },
    });
  },
    ...
    const recentReferrals = recentRows.map((r) => {
      ...
      return {
        ...
        invitedAt: r.invitedAt
          ? (r.invitedAt instanceof Date
              ? r.invitedAt
              : new Date(r.invitedAt as unknown as string)
            ).toISOString()
          : null,
        invitedConverted:
          inviteeEmail.length > 0 && paidEmails.has(inviteeEmail),
      };
    });
...
// Attribution view uses canonical paid plans while retaining legacy Reset and
// Wingman rows. Founder grants remain the beta source of truth until Stripe
// subscription lifecycle syncing replaces this compatibility path.
router.get(
  "/founder/referrals/attribution",
  requireFounder,
  async (req, res): Promise<void> => {
    ...
    const [totalsRow] = await db
      .select({
        ...
        totalPaidConverts: sql<number>`count(${referralsTable.inviteeUserId}) filter (where ${invitee.tier} in ('insight','match','guided','reset','wingman'))::int`,
      })
      .from(referralsTable)
      .leftJoin(invitee, eq(invitee.id, referralsTable.inviteeUserId));
    ...
    const topReferrerRows = await db
      .select({
        ...
        paidConversions: sql<number>`count(${referralsTable.inviteeUserId}) filter (where ${invitee.tier} in ('insight','match','guided','reset','wingman'))::int`,
      })
      .from(referralsTable)
      .leftJoin(inviter, eq(inviter.id, referralsTable.inviterUserId))
      .leftJoin(invitee, eq(invitee.id, referralsTable.inviteeUserId))
    ...
    res.json({
      ...
      totals: {
        ...
        overallConversionRate:
          totalReferrals > 0 ? totalPaidConverts / totalReferrals : 0,
      },
    });
  },
);
  ...
  async (req, res): Promise<void> => {
    const [
      ...
      db
        ...
        .from(usersTable)
        .where(inArray(usersTable.tier, [...PAID_STORED_TIER_VALUES])),
      db
        .select({
    ...
    const ordered = [
      ...
      {
        key: "readiness_gained",
        label: "Gained readiness",
        count: readinessGained,
      },
      {
        key: "entered_matching",
        label: "Entered matching pool",
        count: enteredMatching,
      },
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    const rows = await db
      ...
      .orderBy(
        asc(aiRequestMetricsDailyTable.day),
        asc(aiRequestMetricsDailyTable.toolName),
      );
    ...
    const series = rows.map((r) => {
      ...
      return {
        day:
          typeof r.day === "string"
            ? r.day
            : new Date(r.day as unknown as string).toISOString().slice(0, 10),
        toolName: r.toolName,
        total,
        firstTryOk,
        retriedOk,
...
const OCR_FIELDS: OcrCorrectionField[] = [
  "firstName",
  "age",
  "sourceApp",
  "bio",
...
router.get(
  "/founder/ocr-mismatches",
  requireFounder,
  async (req, res): Promise<void> => {
    const rawWindow =
      typeof req.query.window === "string" ? req.query.window : "";
    ...
    const perField = OCR_FIELDS.map((field) => {
      ...
      if (sortMode === "top") {
        if (b.topDiffCount !== a.topDiffCount)
  ...
  async (req, res): Promise<void> => {
    ...
    for (let d = 0; d <= days; d++) {
      const day = new Date(since.getTime() + d * 24 * 60 * 60 * 1000)
        .toISOString()
        .slice(0, 10);
      const entry = byDayField.get(day) ?? {
        firstName: 0,
        age: 0,
        sourceApp: 0,
        bio: 0,
...
function serializeRule(r: {
  ...
  return {
    ...
    learnedAt:
      r.learnedAt instanceof Date
        ? r.learnedAt.toISOString()
        : String(r.learnedAt),
    updatedAt:
...
router.post(
  "/founder/ocr-pending-rules/:id/approve",
  requireFounder,
  async (req, res): Promise<void> => {
...
router.post(
  "/founder/ocr-pending-rules/:id/reject",
  requireFounder,
  async (req, res): Promise<void> => {
    const id = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
    if (!id) {
...
router.get(
  "/founder/ocr-mismatches/:auditId",
  requireFounder,
  async (req, res): Promise<void> => {
    ...
    res.json({
      ...
      ocrCorrections: (row.ocrCorrections ??
        null) as OcrCorrectionsRecord | null,
      profile: {
...
router.post(
  "/founder/ai-threshold-changes/:id/undo",
  requireFounder,
  async (req, res): Promise<void> => {
    ...
    const hasOld =
      row.oldWindowSize !== null &&
      row.oldMinSample !== null &&
      row.oldThreshold !== null;
    ...
    const current: ThresholdConfig | undefined = existing
      ? {
          windowSize: existing.windowSize,
          minSample: existing.minSample,
...
router.put(
  ...
  async (req, res): Promise<void> => {
    ...
    if (resetGlobal) {
      ...
      await db
        .delete(aiAlertThresholdsTable)
        .where(eq(aiAlertThresholdsTable.toolName, AI_ALERT_GLOBAL_KEY));
      if (prev) {
        changeRows.push({
    ...
    if (removeToolNames && removeToolNames.length > 0) {
      for (const name of removeToolNames) {
        ...
        await db
          .delete(aiAlertThresholdsTable)
          .where(eq(aiAlertThresholdsTable.toolName, name));
        if (prev) {
          changeRows.push({
            toolName: name,
            action: "remove",
...
const putAlertSettingsSchema = z.object({
  rebreachCooldownMinutes: z
    ...
    .max(
      MAX_REBREACH_COOLDOWN_MINUTES,
      `Cannot exceed ${MAX_REBREACH_COOLDOWN_MINUTES} minutes`,
    ),
});
...
router.put(
  ...
  async (req, res): Promise<void> => {
    ...
    await db
      ...
      .values({
        key: FOUNDER_SETTINGS_REBREACH_COOLDOWN,
        value: rebreachCooldownMinutes,
        updatedAt: now,
      })
      .onConflictDoUpdate({
...
router.delete(
  "/founder/alert-settings/rebreach-cooldown",
  requireFounder,
  async (_req, res): Promise<void> => {
    await db
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    const userMessage =
      contextHint && contextHint.trim().length > 0
...
router.get(
  ...
  async (_req, res): Promise<void> => {
    const rows = await db
      ...
      .where(
        inArray(matchPoolMembershipTable.status, ["ready", "concierge_only"]),
      )
...
router.get(
  ...
  async (req, res): Promise<void> => {
    ...
    const tier = resolveCommercialPlan(user.tier).plan.key;
    const createdAt = user.createdAt instanceof Date ? user.createdAt : null;
    const ageDays = createdAt
      ? Math.floor((Date.now() - createdAt.getTime()) / (1000 * 60 * 60 * 24))
      : null;
    const lastAuditAt =
      auditAgg?.lastAt instanceof Date ? auditAgg.lastAt.toISOString() : null;
    ...
    res.json({
      signals: {
        ...
        invitedAt:
          user.invitedAt instanceof Date ? user.invitedAt.toISOString() : null,
      },
    });
  },
);
...
router.put(
  ...
  async (req, res): Promise<void> => {
    ...
    const controls = await saveBrainControls(
      parsed.data as Partial<BrainControls>,
    );
  ...
  async (_req, res): Promise<void> => {
    ...
    const latestPerUser = db
      .select({
        ...
        latest: sql<string>`max(${matchingReadinessSnapshotsTable.day})`.as(
          "latest",
        ),
      })
      .from(matchingReadinessSnapshotsTable)
...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    const [row] = await db
      ...
      .onConflictDoUpdate({
        target: [
          founderCurationTable.entityType,
          founderCurationTable.entityId,
        ],
  ...
  async (req, res): Promise<void> => {
    ...
    res.json({
      reports: rows.map((r) => ({
        ...
        subjectType: r.subjectType ?? "member",
        externalApp: r.externalApp ?? null,
