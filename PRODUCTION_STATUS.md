...
| Founder authorization | Blocked for release | Browser-visible `VITE_FOUNDER_KEY` and the `x-founder-key` API gate remain. Replace with authenticated roles and an immutable actor log. |
...
milestone is completion of Phase 0, with particular priority on founder auth,
database-backed lifecycle tests, payments/entitlements, and synchronized legal
copy. Install the approved five-destination shell only after those trust promises
